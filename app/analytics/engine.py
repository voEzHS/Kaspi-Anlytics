...
def calc_procurement_v2(rows: list[dict], stock_rows: list[dict], channel_rows: list[dict],
    ...
    return {
        ...
        "known_gaps": [
            ...
            "(\"Инстаграм Астана\" и т.п.), сомнений нет. Для канала Каспи (самый крупный, ~9750 из "
            "42к строк) это НЕ подтверждено: живая проверка в CRM (Склад → Бронь товаров) показала "
            "все 72 исторических Kaspi-резерва помечены единообразно \"Каспи (Алматы)\", ни разу "
            "Астана/Шымкент — но лог старый (последняя запись дек. 2024), явно неактивный механизм, "
            "не годится ни подтвердить, ни опровергнуть текущую логику. Каждая рекомендация "
            "в city_transfers несёт kaspi_share_pct — чем выше доля Kaspi в спросе SKU, тем ниже "
            "уверенность, что \"Город канала продаж\" = реальный физический склад отгрузки. "
            "Быстрее всего закрыть — спросить у Дамира напрямую, привязан ли Kaspi-листинг "
            "к конкретному складу города или это единый листинг с последующим ручным распределением",
