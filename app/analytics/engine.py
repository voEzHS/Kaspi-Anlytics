...
def calc_procurement_v2(rows: list[dict], stock_rows: list[dict], channel_rows: list[dict],
    ...
    return {
        ...
        "known_gaps": [
            ...
            "ВАЖНО (08.08, аудит достоверности): \"Город канала продаж\" точно означает город-склад "
            "для розницы/Instagram/TikTok/Магазин — эти каналы сами city-суффиксированы в CRM "
            "(\"Инстаграм Астана\" и т.п.), сомнений нет. ДЛЯ КАНАЛА КАСПИ ЭТО ОПРОВЕРГНУТО (08.08, "
            "прямая проверка по указанию Дамира) — не просто неподтверждено, а доказано неверно: "
            "фильтр CRM Аналитика→Товары по Точка отгрузки=Склад Астана + Канал=Каспи + "
            "Раздел=Коммерческое оборудование (05.08-08.08) дал 1 позицию на 499 500₸; тот же фильтр "
            "БЕЗ Точки отгрузки, только по Город=Астана, дал 75 позиций на 24.18М₸ — то есть "
            "\"Город канала продаж\"=Астана почти никогда не совпадает с реальным складом отгрузки "
            "(Точка отгрузки) для Kaspi. Значит city_transfers для SKU с высокой долей Kaspi в спросе "
            "(kaspi_share_pct) построены на неверной географии и НЕ должны исполняться как есть. "
            "Для розницы/соцсетей проблемы нет — там канал сам содержит город. Исправление требует "
            "нового экспорта из CRM с колонкой \"Точка отгрузки\" вместо \"Город канала продаж\" "
            "(текущий подтверждённый формат файла её не содержит) — нужно попросить Дамира выгрузить "
            "такой отчёт (Аналитика→Товары→фильтр→Excel/Excel (кастом), проверить есть ли там колонка "
            "склада отгрузки) прежде чем доверять city_transfers по Kaspi-тяжёлым SKU",
